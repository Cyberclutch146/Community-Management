...
export const sendOTPEmail = async (to: string, code: string, eventTitle: string) => {
  ...
  return transporter.sendMail({
    from: `"Kindred Relief Network" <${process.env.EMAIL}>`,
    to,
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 12px;">
        ...
        <p style="color: #9ca3af; font-size: 12px; text-align: center;">Kindred Relief Network &bull; Outreach & Relief Coordination</p>
      </div>
    `
  });
};
